...
class HealthBar extends entity.Component {
  ...
  InitComponent() {
    ...
    this._RegisterHandler('health.remove-bar', () => { this._RemoveBar(); });
  }
  
  _RemoveBar() {
    if (this._bar) {
      this._params.parent.remove(this._bar);
      this._bar = null;
    }
  }
  ...
  Update(timeElapsed) {
    ...
    // Check if the parent entity and its position are valid
    if (this._parent && this._parent._position) {
